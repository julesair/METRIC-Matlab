function ETr = ETreference(meteodata,m_i,time,lambda,dRnG_COLD,rah)
% [mm/h]
...
air_density=P*1000/(287.058*(273.15+T));
ETr = 0.408*slope*dRnG_COLD+gamma*Cn/(T+273)*u2*(e_sat-e_act)/(slope+gamma*(1+Cd*u2));
%ETr = ((slope*dRnG_COLD+3600*10^-6*air_density*1000*(e_sat-e_act)/rah)/(slope+gamma*(1+30/rah)))/lambda;
